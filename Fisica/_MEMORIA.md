...
| 04/08/2026 | **Auditoria TikZ da 1ª série · Bloco 1.** Reler o padrão contra figuras já aprovadas encontra violações que a aprovação original deixou passar: seis das dez peças feriam respiro — a pior era a tração desenhada **sobre a própria corda**, e não numa faixa paralela. Refeitas as seis e criadas quatro novas nas lacunas reais do texto: diagrama de corpo livre (a ferramenta padrão da série não tinha figura), par livro/Terra, DCL dos corpos ligados e gráfico $\|F_{el}\| \times x$. Duas lições de método: (1) **figura nova entra como última página da fonte** (`fig-06`, `fig-07`) — a posição no Markdown segue a pergunta pedagógica, não o número do arquivo; renumerar geraria órfãos no repo público, que o publicador não remove; (2) a conferência de coluna de 720 px é a que pega altura excessiva — o primeiro DCL saiu com 912 px e foi comprimido para 824 px, faixa das demais. Reindexar com `publicado_commit` preenchido troca as URLs de `/main/` para commit fixo. Coleção de Física: **107 PNGs**, commit `1addc5b0379a`. |
